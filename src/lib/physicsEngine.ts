...
export function runPhysicsIteration(
  ...
  // Performance Optimization: Consolidated sequential array iterations and replaced closure with standard loop.
  for (let i = 0; i < entities.length; i++) {
    ...
    const movement = Math.hypot(f[0], f[1], f[2]) * config.damping * decay;
    totalMovement += movement;
    
    positions.set(entity.id, [
      pos[0] + f[0] * config.damping * decay,
      pos[1] + f[1] * config.damping * decay,
      pos[2] + f[2] * config.damping * decay,
    ]);
  }
